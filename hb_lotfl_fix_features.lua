-- This file comes from luaotfload-features.lua
-- The change is that features are saved in handle_request function
...
local handle_request = function (specification)
    ...
    local defaults    = apply_default_features(request.features)
    request.features.language = defaults.language
    request.features.script = defaults.script
    ...
    for k,v in pairs(request.features) do
        specification.features[k] = v
    end
    return specification
end
